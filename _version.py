...
def current_version(commits_ahead=False):
    ...
    env = os.environ.copy()
    output = subprocess.run("git describe --tags --long", shell=True, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def create_new_version_tag(new_tag: str, new_version_description: str = "Release Version", update_patch: bool = True):
...
def view_releases():
    env = os.environ.copy()
    output = subprocess.run('git tag -l --sort=-version:refname "v*" -n3', shell=True, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if output.returncode == 0:
...
def get_remote_url():
    env = os.environ.copy()
    output = subprocess.run('git config --get remote.origin.url', shell=True, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if output.returncode == 0:
